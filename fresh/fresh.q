...
/ outputs from fft
fresh.feat.fftaggreg:{[x] getmoment:{[y;moment](("f"$y)$("f"$fresh.arange[0;count y;1])xexp moment)%(sum y)};
 l:fresh.abso[fresh.rfft[x]`]`;
...
fresh.createfeatures2:{[data;aggs;cnames;dict]
 	mkname:{`$"_"sv string x[0],x 2};
 	r:?[data;();aggs!aggs:aggs,();mkname'[comb]!1_'comb:(flip(key;value)@\:fresh.getsingleinputfeatures[])cross cnames];
 	if[count mcols@:where 98=type each value[r]mcols:exec c from meta[r]where null t;
		r:key[r]!(mcols _ value r){x,'y}/{(`$"_"sv'string x,'cols y)xcol y}'[mcols;value[r]mcols]];
	values:{{{{{value (.ml.fresh.feat[y]),/(((enlist enlist x),z))}[x;y]each (cross/)z y}[x;;y]each key y}[x;y]}[;z]each x y};
	names:{raze key[x]cross'(cross/)each value each value x};
	r1:?[data;();aggs!aggs:aggs,();({`$"_"sv string x} each cross[cnames;til count i.namemap::names[dict]])!{raze raze x}values[data;cnames;dict]];
	if[count mcols@:where 98=type each value[r1]mcols:exec c from meta[r1]where null t;
 		r1:key[r1]!(mcols _ value r1){x,'y}/{(`$"_"sv'string x,'cols y)xcol y}'[mcols;value[r1]mcols]];
	key[r]!value[r] ^ value r1
	}
...
pdictvals:`binnedentropy`cidce`numcrossingm`ratiobeyondrsigma`largestdev`c3`autocorr`indexmassquantile`numcwtpeaks`numpeaks`symmetriclooking`treverseasymstat`quantile`rangecount`changequant`eratiobychunk`fftcoeff
